...
static std::shared_ptr<arrow::Table> generate_data_frame(int64_t num_vectors, torch::Tensor ids) {
    ...
    for (int64_t i = 0; i < num_vectors; i++) {
        price_builder.Append(static_cast<double>(rand()) / RAND_MAX * 100.0); // random number between 1 and 100
...
class QuakeSerialFlatBenchmarkGlobalBuild : public ::testing::Test {
    protected:
        std::shared_ptr<QuakeIndex> index_;
        Tensor data_;
        Tensor ids_;
...
TEST_F(QuakeSerialFlatBenchmark, GraphForLocalPre) {
    ...
    auto total_time_ns = 0;

    auto start = high_resolution_clock::now();
    std::ofstream outfile("local_pre_graph.txt");
    for(int filter_price = 0; filter_price <= 100; filter_price += 1) {
        total_scan_time = 0;
        total_filter_time = 0;
        total_time_ns = 0;
        ...
        // std::cout << "Selectivity:" << filter_price<<std::endl; 
        // std::cout << "Average Filter Time: " << total_filter_time/queries.size(0) << " ms" << std::endl;
        // std::cout << "Average Scan Time: " << total_scan_time/(queries.size(0)) << " ms" << std::endl;
        // std::cout << "Average Lantency Time: " << total_time_ns/(queries.size(0)) << " ms" << std::endl << std::endl;
        // std::cout<< total_scan_time / 1000000 << std::endl;
        // outfile << total_scan_time/1000000 << std::endl;
        outfile << filter_price <<","<<total_filter_time/queries.size(0)<<","<<total_scan_time/(queries.size(0))<<","<<total_time_ns/(queries.size(0))<<std::endl;
    }
    outfile.close();
...
TEST_F(QuakeSerialFlatBenchmark, GraphForLocalPost) {
    ...
    std::cout<<"Local Post Results"<<std::endl;
    for(int filter_price = 0; filter_price <= 100; filter_price += 1) {
        total_scan_time = 0;
        total_filter_time = 0;
        total_time_ns = 0;
...
TEST_F(QuakeSerialFlatBenchmarkGlobalBuild, GraphForBruteForce) {
// Force "serial_scan" by disabling batched_scan
    Tensor queries = generate_data(100, DIM);
    auto search_params = std::make_shared<SearchParams>();
    search_params->k = 10;
...
TEST_F(QuakeSerialFlatBenchmarkGlobalBuild, GraphForGlobalPre) {
    // Force "serial_scan" by disabling batched_scan
    Tensor queries = generate_data(100, DIM);
    auto search_params = std::make_shared<SearchParams>();
    search_params->k = 10;
    ...
    std::cout<<"Global Pre Results"<<std::endl;
    for(int filter_price = 0; filter_price <= 100; filter_price += 1) {
        total_scan_time = 0;
        total_filter_time = 0;
        total_time_ns = 0;
...
TEST_F(QuakeSerialFlatBenchmarkGlobalBuild, GraphForGlobalPost) {
    // Force "serial_scan" by disabling batched_scan
    Tensor queries = generate_data(100, DIM);
    auto search_params = std::make_shared<SearchParams>();
    search_params->k = 10;
    ...
    std::cout<<"Global Post Results"<<std::endl;
    for(int filter_price = 0; filter_price <= 100; filter_price += 1) {
        total_scan_time = 0;
        total_filter_time = 0;
        total_time_ns = 0;
